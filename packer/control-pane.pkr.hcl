...
source "amazon-ebs" "control-pane" {
  ami_name      = "control-pane-ami"
...
build {
  sources = ["source.amazon-ebs.control-pane"]
  provisioner "ansible" {
    playbook_file   = "./playbook_control_pane.yml"
    user="ec2-user"
  }
}